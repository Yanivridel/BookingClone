...
async function generateVerificationCode(email, isLogin) {
    try {
        const verificationCode = crypto_1.default.randomBytes(3).toString('hex').replace(/[a-z]/g, (match) => match.toUpperCase()); // 6-character hex code
        const expiresAt = new Date();
        expiresAt.setMinutes(expiresAt.getMinutes() + 5);
        // Check if the user already has a document
        let verificationRecord = await verificationModel_1.verificationModel.findOne({ email });
        if (verificationRecord) {
            ...
            verificationRecord = new verificationModel_1.verificationModel({
                email,
                verificationCode,
                expiresAt,
            });
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px; background-color: #f0f0f0;">
            ...
            <h3 style="color: #003a63; font-size: 20px;">Email Verification and Account ${isLogin ? "Login" : "Sign Up"}</h3>
            <p style="font-size: 16px; color: #333;">Hello,<br><br>This is your unique verification code for ${isLogin ? "logging in" : "signing up"} to Booking.com. Please enter it within the next 5 minutes.</p>
            ...
            <p style="font-size: 12px;">© Booking.com 2025. All rights reserved.</p>
            </div>
        </body>
        </html>
        `,
...
async function verifyVerificationCode(email, enteredCode) {
    try {
        const verificationRecord = await verificationModel_1.verificationModel.findOne({ email });
        if (!verificationRecord) {
        ...
        await verificationModel_1.verificationModel.deleteOne({ email });
        return true;
    }
    catch (error) {
