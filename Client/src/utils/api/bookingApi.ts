...
export async function savePayment(amount: number, currency: string, transactionId: string, paymentMethod: string) {
    try {
        const { data } = await axios.post(`${API_URL}/api/payment/save`, 
            { 
                amount, 
                currency,
                transactionId,
...
export async function getPaymentMethod(paymentId: string) {
    try {
        ...
        const { data } = await axios.get(`https://api.stripe.com/v1/payment_methods/${paymentId}`, {
            headers: {
