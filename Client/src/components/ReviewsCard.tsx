...
function ReviewsCard({ review, className, size }: propertyReviewsProps) {
  return (
      <Card key={review._id} 
      className={cn(`h-[210px] flex ${size === 1 ? "py-1 px-4" : "p-5"}`, className)}>
        <div className="h-[50%] grid gap-3">
          <div className="flex gap-2 items-center">
            <div>
              <Avatar className={ size===1 ? "w-6 h-6": "w-8 h-8"}>
                <AvatarImage src={(review.userId as IUser).user_image} />
                <AvatarFallback>
                  <img 
            ...
            <div>
              <div>
                <b className={ size===1 ? "text-sm": "text-lg"}>{(review.userId as IUser).fName || "Anonymous"}</b>
              </div>
              <div className="flex items-center gap-1">
                <Avatar className={ size===1 ? "w-3 h-3": "w-5 h-5"}>
                  <AvatarImage src={"https://t-cf.bstatic.com/design-assets/assets/v3.138.1/images-flags/Il.png"} />
                </Avatar>
