...
function PropertyCard({ propertyData, isGrid }: PropertyCardProp) {
  const selectedRooms = (propertyData.rooms as IRoom[])
    .filter((room) =>
      propertyData.selectedRooms?.some((selected) => selected.id === room._id)
    )
    .map((room) => {
      const selected = propertyData.selectedRooms?.find(
        (selected) => selected.id === room._id
      );
  ...
  const isLimited = selectedRooms.some(
    (room) =>
      new Date(room.offers[0].discount.expires).getTime() <=
          ...
          <div className="flex h-full flex-col justify-between">
            <div>
              <CardContent className="">
                <CardTitle
                  className="text-blue-600 text-lg font-bold hover:text-black cursor-pointer"
                ...
                <div className="flex gap-1 p-1">
                  <div className="flex">
                    {Array(Math.round((propertyData.total_rating || 0) / 2))
                      .fill(0)
                      .map((_, index) => (
                ...
                <CardDescription className="text-xs">
                  <div className="flex items-center gap-1 p-1">
                    ...
                    <CardTitle>
                      {getDescByRating(propertyData.total_rating || 0) + ", "}
                    </CardTitle>
                    <CardDescription className="text-xs">
                      reviews: {propertyData.reviews_num}
                    </CardDescription>
                  </div>
              ...
              <div
                className={`ps-3 ms-5 ${isRtl ? "border-r-2" : "border-l-2"}`}
              >
                {selectedRooms.map((room, idx) => (
                  <div key={room._id + idx} className="relative ps-2">
                    <p
                      className={`absolute rounded-lg text-black text-xs px-[1px] py-[2px] bg-[#f2f2f2] ${
                    ...
                    <div className="text-xs flex gap-2">
                      {room.rooms
                        .map((r) => {
                          ...
                          if (r.beds.double)
                            str += r.beds.double + " double bed ";
                          if (r.beds.single)
                            str += r.beds.single + " single bed ";
                    ...
                    {new Date(room.offers[0].discount.expires).getTime() <=
                      new Date().setDate(
                        new Date().getDate() + DAYS_FOR_LIMITED
                      ) && (
                    ...
                    {room.available.availableRooms <= ROOMS_FOR_LIMITED && (
                      <p className="text-red-600 text-xs font-medium">
                        Only {room.available.availableRooms} left at this price
                        on our site
                      </p>
                    )}
                  </div>
            ...
            <div dir={isRtl ? "ltr" : "rtl"} className="grid gap-1 p-4">
              <div className="text-xs">
                {propertyData.selectedRooms &&
                  new Date(
                    propertyData.selectedRooms[0].available.startDate
              ...
              <Button onClick={handleNavToProperty}>
                {"<"} See availability
              </Button>
            </div>
          </div>
        ...
        <Card className="flex justify-between p-5 mx-auto max-w-[815px]">
          <div className="flex gap-5">
            ...
            <div className="flex flex-col gap-2 max-w-[50%]">
              <CardTitle>
                <span
                  className="text-blue-600 text-xl hover:text-black cursor-pointer"
                  onClick={handleNavToProperty}
                >
                  {cf(propertyData.title)}
                </span>
              ...
              <CardDescription>
                <div className="flex gap-2">
                  <p className="cursor-pointer text-blue-500 text-xs">
                    <u>
                      {propertyData.location.addressLine + ","}
                      {propertyData.location.city + ","}
                    </u>
                ...
                {isLimited && (
                  <Badge variant="deals" className="mt-1">
                    Limited-time Deal
                  </Badge>
                )}
              </CardDescription>
